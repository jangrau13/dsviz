...
# A name that is not a function must not silently fall back either.
src = f"{FUNCS}\n{WORLD}\njob = MapReduce(map=nosuch, reduce=total, partition=byKey)\nworld.run(job)\n"
got = verdict("t1-wordcount", src)
ok("a job naming a function that does not exist is refused",
   got["verdict"] == "CE", got["cases"][0]["message"][:70])
