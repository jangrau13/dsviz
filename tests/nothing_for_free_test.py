...
for omitted in ("map", "reduce", "partition"):
    ...
    src = f"{FUNCS}\n{WORLD}\njob = MapReduce({wiring}, partitions=2)\nworld.run(job)\n"
...
# A call to something that is not a function is refused, wherever it appears.
STRAY = FUNCS + "\ndef sideways(key: string, value: string) -> [pair]:\n    return [(nosuchcall(key), 1)]\n"
src = f"{STRAY}\n{WORLD}\njob = MapReduce(map=perDay, reduce=addUp, partition=spread, partitions=2)\nworld.run(job)\n"
got = verdict("fx-takings", src)
message = got["cases"][0]["message"]
ok("a call to a function nobody declared is refused",
   got["verdict"] == "CE" and "nosuchcall" in message,
   message[:70])

# A decorator the runtime does not know declares machines no world can hold,
# so it is refused rather than left to invent a cluster nobody wrote.
BAD_KIND = FUNCS + '''
@worker
class Till:
    pass

t1 = Till(type="m1.small")
world = World(machines=[t1])

job = MapReduce(map=perDay, reduce=addUp, partition=spread, partitions=2)
world.run(job)
'''
got = verdict("fx-takings", BAD_KIND)
message = got["cases"][0]["message"]
ok("a decorator that is not a kind of class is refused",
   got["verdict"] == "CE" and "@worker is not a kind of class" in message,
   message[:70])
...
broken = [n for n, s in ASSIGNMENTS.items()
          if not (s.expects or s.requires)
